    ...
    return new Promise((resolve, reject) => {
      ...
      client.on('data', data => {
        ...
        try {
          ...
          if (parsed.messageId) {
            console.log(
              `📨 客户端 ${clientId} 收到响应: ${parsed.messageId} (${parsed.success ? '成功' : '失败'})`
            );
          }
        } catch (e) {
  ...
  async testBasicConnectivity() {
    ...
    try {
      ...
      const response = client.messages.find(
        m => m.command === 'ping' || m.data?.message === 'pong'
      );
  ...
  async testJsonProtocol() {
    ...
    try {
      ...
      for (const msg of client.messages) {
        if (
          msg.messageId &&
          Object.prototype.hasOwnProperty.call(msg, 'success') &&
          msg.timestamp &&
          msg.version
        ) {
          validResponses++;
      ...
      if (validResponses >= commands.length) {
        this.testResults.push({
          test: 'json-protocol',
          result: 'PASS',
          details: `${validResponses} valid responses`,
        });
    ...
    try {
      ...
      if (successfulClients === clientCount) {
        this.testResults.push({
          ...
          result: 'PASS',
          details: `${successfulClients} clients successful`,
        });
    ...
    try {
      ...
      if (errorResponses.length >= 2) {
        this.testResults.push({
          ...
          result: 'PASS',
          details: `${errorResponses.length} error responses`,
        });
  ...
  async testPerformance() {
    ...
    try {
      ...
      if (avgResponseTime < 100 && maxResponseTime < 200) {
        ...
        throw new Error(
          `响应时间超出预期: avg=${avgResponseTime.toFixed(2)}ms, max=${maxResponseTime}ms`
        );
      }
